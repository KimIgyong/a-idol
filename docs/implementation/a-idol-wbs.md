...
| T-089 | 🟡 20% | identity (pilot) + admin-ops 모듈 완료. identity: signup/login/refresh/logout/update-me (`device_id`, `refresh_token`, `avatar_url`, `marketing_opt_in`, `push_opt_in`). admin-ops: AdminRefreshDto.refreshToken → refresh_token (admin-auth refresh/logout). CMS use-logout body + ITC admin-auth.spec 6건 + auth.spec/security.spec/update-me.spec atomic. 129/129 ITC pass. **잔여 8개 모듈** (catalog/fandom/chat/commerce/audition/photocard/design-assets/project-docs). |
