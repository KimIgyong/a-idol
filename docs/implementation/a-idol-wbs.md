...
| T-084 | 🟢 96% | ITC 86 → **129** (+43). MYVOTES/AUTH-rotation/logout/password/lockout/unlock/ADMIN-AUTH/IDOL-AUTHZ/ADMIN-PHOTOCARD-AUTHZ/ADMIN-CATALOG-AUTHZ/**ADMIN-DESIGN-ASSETS-AUTHZ**(4)/**ADMIN-PROJECT-DOCS-AUTHZ**(4)/CSP/SECURITY. harness `setGlobalPrefix('api')` 누락 fix + operator login upsert. |
...
| T-089 | 🟡 70% | identity + admin-ops + catalog + fandom (no-op) + chat (no-op) + commerce + audition 완료 (7/10). audition: idol_ids/start_at/end_at/order_index/max_advancers/heart_weight/sms_weight/ticket_weight/daily_heart_limit + vote module CastVoteBody.idol_id. CMS audition admin-api boundary transform + mobile castVote + ITC 5 spec + audition-fixtures helper. 129/129 ITC + 57/57 mobile test pass. **잔여 3개 모듈** (photocard/design-assets/project-docs). | commerce: priceKrw/deliveryPayload/isActive/productId/providerTxId/receiptJws → snake_case. fandom/chat 은 단일 토큰 필드만이라 변환 없음. shared CreatePurchaseDto + mobile useCommerce + CMS createProduct/updateProduct + ITC commerce/photocard/vote/audition/authz spec atomic. 129/129 ITC pass. **잔여 4개 모듈** (audition/photocard/design-assets/project-docs). | identity: signup/login/refresh/logout/update-me. admin-ops: AdminRefreshDto. catalog: CreateAgency/UpdateAgency/CreateIdol/UpdateIdol/CreateSchedule/ListAdminIdolsQuery (`agency_id`, `stage_name`, `hero_image_url`, `publish_immediately`, `start_at`, `end_at`, `include_deleted`). CMS API 보더리에서 camelCase ↔ snake_case 변환 (UI form 은 camelCase 유지 — 보더리 transform 패턴). 129/129 ITC pass. **잔여 7개 모듈** (fandom/chat/commerce/audition/photocard/design-assets/project-docs). |
