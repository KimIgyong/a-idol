...
| T-089 | 🟡 80% | identity + admin-ops + catalog + fandom(no-op) + chat(no-op) + commerce + audition + photocard 완료 (8/10). photocard: idolId/isActive/imageUrl/dropWeight → snake_case (CreatePhotocardSet/UpdatePhotocardSet/AddPhotocardTemplate). admin-photocard.controller 3 매핑 + CMS createPhotocardSet/updatePhotocardSet/addPhotocardTemplate boundary transform + admin-photocard-authz spec. 129/129 ITC + 57/57 mobile pass. **잔여 2개 모듈** (design-assets/project-docs). |
