...
| T-089 | 🟡 10% | ADR-023 작성 + identity 모듈 pilot 완료 (signup/login/refresh/logout/update-me — `device_id`, `refresh_token`, `avatar_url`, `marketing_opt_in`, `push_opt_in`). backend DTO + 컨트롤러 매핑 + shared `UpdateUserMeDto` + mobile client + AuthContext + extra 화면 + ITC (auth.spec, security.spec, update-me.spec) 모두 atomic 갱신. 24/24 ITC + 11/11 mobile test pass. **잔여 9개 모듈** (admin-ops/catalog/fandom/chat/commerce/audition/photocard/design-assets/project-docs). |
