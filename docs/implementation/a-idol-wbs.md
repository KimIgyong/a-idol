...
| T-084 | 🟢 99% | ITC 86 → **129** (+43) + 신규 unit test 53건 (design-asset 6 + project-doc 10 + agency 8 + create-idol 6 + schedule 7 + grant-coupon 6 + get-chat-balance 7 + open-room 3 — **182 unit total**). RBAC ITC + catalog 비즈니스 룰 unit + chat 쿠폰/잔액/멤버십 게이트 unit: COUPON_INVALID_DELTA, KST midnight (UTC+9 no-DST), nextResetAt, CHAT_GATE_NOT_MEMBER, ADMIN_GRANT 기본값. design-asset / project-doc: ConflictException + NotFoundException + version++. harness `setGlobalPrefix('api')` fix + operator upsert. |
