...
| T-084 | 🟢 99.5% | ITC 86 → **129** (+43) + backend unit **268** (+16 chat: 4 cancel-auto-message + 6 list-auto-messages + 6 list-messages) + CMS Vitest+RTL 29 + mobile **66** (+9: 4 useChatBalance + 5 useCheers). backend: audition state machine, vote-weight validation, admin-ops 보안, **chat auto-message 상태머신 (DISPATCHED 거부 / CANCELED idempotent / SCHEDULED→cancel) + list pagination clamp + messages 룸 소유권 가드**, catalog/identity/photocard/design-asset/project-doc 비즈니스 룰. CMS: WBS 파서, react-markdown, zustand auth, apiFetch ETag. mobile: useChatBalance refresh/error/null token, useCheers post optimistic + 빈 메시지 차단 + loadMore append. 합계 **297 unit + 129 ITC + 66 mobile = 492 tests pass**. |
