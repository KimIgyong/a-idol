...
| T-084 | 🟢 98% | ITC 86 → **129** (+43) + 신규 unit test 37건 (design-asset 6 + project-doc 10 + agency 8 + create-idol 6 + schedule 7 — **166 unit total**). RBAC ITC: MYVOTES/AUTH-rotation/logout/password/lockout/unlock/ADMIN-AUTH/IDOL-AUTHZ/ADMIN-PHOTOCARD-AUTHZ/ADMIN-CATALOG-AUTHZ/ADMIN-DESIGN-ASSETS-AUTHZ/ADMIN-PROJECT-DOCS-AUTHZ/CSP/SECURITY. catalog 비즈니스 룰 unit 커버: AGENCY_NOT_FOUND/AGENCY_HAS_IDOLS, IDOL_NOT_FOUND, INVALID_SCHEDULE_RANGE, SCHEDULE_NOT_FOUND, mbti 대문자/4자, name/description trim. design-asset / project-doc: ConflictException + NotFoundException + version++ (content 변경). harness `setGlobalPrefix('api')` fix + operator upsert. |
