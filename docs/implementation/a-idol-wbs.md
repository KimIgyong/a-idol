...
| T-084 | 🟢 99.5% | ITC 86 → **129** + backend unit **311** (+17 vote: 9 list-my-votes + 3 my-tickets + 5 my-vote-status) + CMS Vitest+RTL 29 + mobile **77** (+11: 6 useAuditions + 5 useMyVotes). backend vote: pagination clamp (size 1~50 + page floor) + nextCursor 계산 + global/round-scoped ticket balances + dailyHeartLimit + KST midnight resetAt + ROUND_NOT_FOUND. audition state machine, chat auto-message, catalog/identity/photocard/design-asset/project-doc 비즈니스 룰. CMS: WBS 파서, react-markdown, zustand auth, apiFetch ETag. mobile: useChatBalance / useCheers / **useAuditionsList (status filter) + useAudition (id 가드) + useMyVotes (loadMore + nextCursor + null token skip)**. 합계 **340 unit + 129 ITC + 77 mobile = 546 tests pass**. |
