...
| T-084 | 🟢 99.5% | ITC 86 → **129** (+43) + 신규 unit test **102건 (231 unit total)**: design-asset 6 + project-doc 10 + agency 8 + create-idol 6 + schedule 7 + grant-coupon 6 + get-chat-balance 7 + open-room 3 + unlock-account 3 + list-operators 2 + get-admin-me 2 + logout-admin 4 + refresh-admin-token 8 + **vote-rule 13 + round 17**. audition state machine + parent guard 커버: SCHEDULED→ACTIVE 전 parent !ACTIVE → AUDITION_NOT_ACTIVE, ACTIVE→CLOSED → ROUND_CLOSED 이벤트, vote-weight validation (negative + all-zero + dailyHeartLimit default), endAt<=startAt → AUDITION_INVALID_DATE_RANGE, soft-deleted audition → AUDITION_NOT_FOUND. admin-ops 보안 + chat 쿠폰/잔액/멤버십 + catalog 비즈니스 룰 모두 커버. |
