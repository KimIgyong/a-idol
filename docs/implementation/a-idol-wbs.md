...
| T-084 | 🟢 99% | ITC 86 → **129** (+43) + 신규 unit test 72건 (design-asset 6 + project-doc 10 + agency 8 + create-idol 6 + schedule 7 + grant-coupon 6 + get-chat-balance 7 + open-room 3 + unlock-account 3 + list-operators 2 + get-admin-me 2 + logout-admin 4 + refresh-admin-token 8 — **201 unit total**). admin-ops 보안 비즈니스 룰 unit 커버: refresh rotation + 7가지 reject 경로 (verify fail / missing session / revoked / expired / **hash mismatch → defensive revoke** / admin 삭제 / suspended → INVALID_CREDENTIAL). chat 쿠폰/잔액/멤버십 게이트, catalog 비즈니스 룰 (AGENCY_NOT_FOUND/AGENCY_HAS_IDOLS/IDOL_NOT_FOUND/INVALID_SCHEDULE_RANGE) 모두 커버. harness `setGlobalPrefix('api')` fix + operator upsert. |
