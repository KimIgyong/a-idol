...
| T-084 | 🟢 97% | ITC 86 → **129** (+43) + 신규 unit test 16건 (design-asset usecases 6 + project-doc usecases 10 — 145 unit total). MYVOTES/AUTH-rotation/logout/password/lockout/unlock/ADMIN-AUTH/IDOL-AUTHZ/ADMIN-PHOTOCARD-AUTHZ/ADMIN-CATALOG-AUTHZ/**ADMIN-DESIGN-ASSETS-AUTHZ**(4)/**ADMIN-PROJECT-DOCS-AUTHZ**(4)/CSP/SECURITY. design-asset / project-doc 모듈은 ConflictException(slug 중복) + NotFoundException + version++(content 변경 시) 까지 전부 unit test 커버. harness `setGlobalPrefix('api')` 누락 fix + operator login upsert. |
