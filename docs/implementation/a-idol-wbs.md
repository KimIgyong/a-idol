...
| T-084 | 🟢 99.5% | ITC 86 → **129** + backend unit **294** (+26 audition.usecases) + CMS Vitest+RTL 29 + mobile **66**. backend: **audition Create/List/Get/Update/Transition/Delete + AddEntries/RemoveEntry 26 TC** (date range, idolIds 검증 + IDOL_NOT_FOUND validate-first + name/description trim, publicOnly 시 DRAFT 거부 / ACTIVE+FINISHED 노출, FINISHED→ACTIVE 거부, DRAFT-only 삭제 + 엔트리 제거, parent touchUpdatedAt). + 기존 chat auto-message 상태머신 / list pagination / 룸 소유권 가드, catalog/identity/photocard/design-asset/project-doc 비즈니스 룰. CMS: WBS 파서, react-markdown, zustand auth, apiFetch ETag. mobile: useChatBalance, useCheers. 합계 **323 unit + 129 ITC + 66 mobile = 518 tests pass**. |
