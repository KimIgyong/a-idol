...
| 2026-04-27 | T-084 — `design-asset.usecases.spec` (6) + `project-doc.usecases.spec` (10) 신규 unit test. 129 → **145 unit pass**. ConflictException(slug 중복) / NotFoundException(missing id+slug) / version++ (content 변경) 모두 커버. T-084 96% → 97%. |
| 2026-04-27 | T-089 / ADR-023 — design-assets + project-docs 모듈 완료 (**10/10, T-089 closed**). design-assets: file_url/order_index. project-docs: source_type/source_path/order_index. backend DTO + 컨트롤러 매핑 + shared 4개 interface (CreateDesignAssetDto/UpdateDesignAssetDto/CreateProjectDocDto/UpdateProjectDocDto). CMS design-assets-page + deliverables-page 의 form state 도 shared 타입 직접 사용해 snake_case. ITC 2건 atomic. **129/129 ITC + 57/57 mobile + 4 workspace typecheck pass**. ADR-023 미션 종료. T-089 80% → 🟢 100%. |
| 2026-04-27 | T-089 / ADR-023 — photocard 모듈 완료 (8/10). photocard.dto.ts CreatePhotocardSet/UpdatePhotocardSet/AddPhotocardTemplate 의 `idolId`/`isActive`/`imageUrl`/`dropWeight` → snake_case. admin-photocard.controller 3 매핑 + CMS createPhotocardSet/updatePhotocardSet/addPhotocardTemplate boundary transform + admin-photocard-authz spec. 129/129 ITC pass. T-089 70% → 80%. |
