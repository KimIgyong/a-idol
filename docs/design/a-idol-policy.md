...
## POL-011: Fan Level & Gamification Policy (팬 레벨·게임화 정책)

- **Purpose**: 장기 리텐션·활동 다양성 증진.
- **Scope**: FR-080..084, FR-170, FR-171.
- **Rules**:
  1. 레벨은 10단계 (Lv.1 뉴팬 ~ Lv.10 아이돌메이커). 누적 XP 기준은 `aidol_fan_levels` 표 준수.
  2. XP 획득 활동과 일일 상한·쿨다운은 `aidol_xp_activity_configs` 에서 관리(CMS FR-170).
  3. **레벨 하락 없음** — 달성 후 유지(어뷰징 외 감점 없음).
  4. **혜택 자동 적용**: 채팅 쿼터 보너스(+1/+2/+3), 투표권·가차 할인 쿠폰, 얼리 액세스, 전용 뱃지/프레임.
  5. **스팸 방지**: 채팅 XP 5분 쿨다운, 일일 활동별 상한, 비정상 급등 탐지(자동 24h 정지 + 관리자 검토).
  6. **스트릭(연속 출석)**: 7/30/100/365 일 마일스톤 보상 지급. 1일 공백 → 0 리셋.
  7. **이벤트 보너스**: CMS 에서 한시적 XP 배수·이벤트 활동 추가 가능(audit log 필수).
- **Exceptions**: 신규 유저 온보딩 기간(가입 후 7일)은 레벨 진입 보너스 XP 100 지급.
